...
class Embedding:
    ...
    def get_subembed(self, word_list, **kwargs):
        ...
        #print("vectors: {}".format(self.m.shape))
        #print("wi: {}".format(self.wi))
        
        keep_indices = [self.wi[word] for word in word_list]
        
        #print("\t indices: {}".format(keep_indices))
    ...
    def write_emb(self):
        ...
        print("\tvecs shape: {}".format(self.m.shape))
        print("\t\t{}".format(self.m[0:N, :]))
        print("\tnr of words: {}".format(len(self.wi)))
        print("\twords: {}".format(list(self.wi.keys())))
...
class SVDEmbedding(Embedding):
    ...
    def __init__(self, path, normalize=True, eig=0.0, **kwargs):
        ...
        ut = np.load(path + '.svd.ut.npy', mmap_mode="c")
        s = np.load(path + '.svd.s.npy', mmap_mode="c")
        vocabfile = path + '.svd.words.vocab.pkl'
        #self.iw = open(vocabfile,"r").read().split("\n")
        self.iw = load_pickle(vocabfile)
