...
from representations.sequentialembedding import SequentialEmbedding, SequentialSVDEmbedding
...
def load_embeddings(filename=None):
    ...
    with embed_lock:
        ...
        (start_year, end_year, time_inc, emb_type) = get_sequence_info(filename)
        ...
        if emb_type == "SVD":
            embeddings = SequentialSVDEmbedding.load(filename, range(start_year, end_year, time_inc))
        else:
            embeddings = SequentialEmbedding.load(filename, range(start_year, end_year, time_inc))
        print("LOAD EMBEDDINGS TOOK %s" % (time.time() - start))

        print("Embeddings: ")
        for year in range(start_year, end_year, time_inc):
            print("\t{} => ".format(year))
            embeddings.get_embed(year).write_emb()
...
def get_sequence_info(dir_name):
    
    emb_type = "SGNS"
    if "_svd" in dir_name:
        emb_type = "SVD"
    
    files = glob.glob(dir_name + "/*.npy")
    years = list(map(lambda x: get_year(x, "-w.npy"),files))
    start_year = min(years)
    end_year = max(years)
    time_inc = int((end_year - start_year)/(len(years) - 1))

    return (start_year, end_year + 1, time_inc, emb_type)

def get_year(filename,p):
    
    patt = re.match("^.*\/(\d+)" + p, filename)
    if patt:
        return int(patt.group(1))
    
    print("No year found in file name: {}".format(filename))
    sys.exit(0)
