...
def write_response(handler, code, headers, data=""):
    ...
    if data:
        ...
        content = zlib_encode.compress(data.encode("utf8")) + zlib_encode.flush()
    ...
    if not word1 in search_cache:
        ...
        for i in range(len(words)):
            ...
            obj = {
                ...
                #}
                "similarity" : list(map(float, all_sims[word])),
                "avg_similarity" : float(sum(all_sims[word]) / len(all_sims[word])),
                "sum_similarity" : float(sum(all_sims[word])),
                "position" : {
                    "x" : float(round(fitted[i][0], 3)),
                    "y" : float(round(fitted[i][1], 3))
                }
            }
...
def do_get(handler):
    ...
    if parts.path in STATIC:
        with open(STATIC[parts.path], "r", encoding="utf8") as f:
        #with open(STATIC[parts.path], "r", encoding="unicode_escape") as f:
        ...
        if msg_dict["results"]:
            ...
            print("No results for term *{}* (possibly OOV)".format(msg_dict["term"]))
